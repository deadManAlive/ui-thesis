...
Lalu dengan melakukan substitusi persamaan @binvacuum ke persamaan @ceinvacuum didapatkan persamaan @cceinvacum
...
Persamaan-persamaan @gaussvacuum sampai @binvacuum juga dapat dijabarkan ke dalam domain frekuensi dengan bantuan transformasi Fourier temporal $cal(F)[bup(F)(bup(r), t)] = tilde(bup(F))(bup(r), omega)$ didapatkan persamaan @fourb hingga @foure
...
Jika transformasi Fourier dilakukan pada persamaan @ewaveequation, akan didapatkan persamaan @helmholtz
...
Indeks refraksi $n$ merupakan sebuah skalar yang berupa perbandingan antara cepat rambat gelombang elektromagnetik pada vakum $c$ dan kecepatan fasa pada material $v$ seperti pada persamaan @refrin
...
Jika persamaan @lkexpansion disubstitusikan ke persamaan @nhhelmholtz, maka akan didapatkan persamaan @a1
...
#[
  ...
  Dapat dilihat bahwa suku riil dan imajiner dari persamaan tersebut bernilai 0, sehingga dengan mengevaluasi bagian riil dari persamaan @lkexpanded akan didapakan persamaan @preikonal
  ...
  yang mana persamaan @preikonal tersebut dapat disederhanakan menjadi persamaan @eikonal
...
Salah satu teorema dalam kalkulus vektor adalah bahwa vektor gradien tegak lurus dengan kurva atau permukaan ketinggian. Oleh karena itu, $bup(p)$ sebagai gradien dari permukaan ketinggian dari $S$ akan selalu tegak lurus dengan permukaan tersebut. Hal ini juga dapat dibuktikan dengan menjabarkan vektor Poynting $bup(S)$ rata-rata dari medan elektromagnetik ditunjukkan oleh persamaan @poynting0

$ angle.l bup(S) angle.r = 1/(2mu_0) cal(Re)[bup(E) times dash(bup(B))] $ <poynting0>

\

Pertama, dengan melakukan pendekatan pada persamaan @lkexpansion untuk nilai $omega$ yang sangat besar sehingga deret hanya menyisakan suku ke-nol sebagai suku yang signifikan seperti yang terlihat pada persamaan @limelexp

$
  lim_(omega arrow infinity) tilde(bup(E))(bup(r), omega) = tilde(bup(E))_0(bup(r)) e^(-j k phi.alt(bup(r)))
$ <limelexp>

yang dapat disubstitusikan ke persamaan @fourb (persamaan Gauss) dan @fourfar (persamaan Faraday) serta menggunakan identitas kalkulus vektor $nabla dot (psi bup(F)) = psi nabla dot bup(F) + bup(F) dot nabla psi$ dan $nabla times (psi bup(F)) = psi (nabla times bup(F))+(nabla psi) times bup(F)$ untuk mendapatkan persamaan @eigauss
...
#[
  ...
  dan persamaan @eifaraday menjadi persamaan @pseoiszerotwo
  ...
  Persamaan @pseoiszero membuat suku kedua dari @poynting1 bernilai nol, sedangkan $nabla ps$ adalah riil, selain itu juga terdapat kerapatan listrik rata-rata yang terdefinisi seperti pada persamaan @elintencity
...
Persamaan @finalpoynting tersebut menunjukkan bahwa vektor Poynting rata-rata waktu $angle.l bup(S) angle.r$, sebagai vektor yang menunjukkan arah energi dari gelombang elektromagnetik, memiliki arah yang sama dengan eikonal $nabla phi.alt (bup(r))$, sebagai vektor yang menunjukkan arah propagasi suatu titik di muka gelombang. Implikasi dari hal ini adalah bahwa propagasi gelombang sebagai aliran energi dapat direpresentasikan sebagai garis-garis sinar. Hal inilah yang mendasari GO sebagai analisis propagasi gelombang elektromagnetik dalam bentuk representasi sinar-sinar.

\

== Atenuasi Ruang

Suku imajiner dari persamaan @lkexpanded akan memberikan persamaan @thehell
...
#[
  ...
  Suku $(nabla pr dot nabla)e0$ pada persamaan @thehell itu merupakan aplikasi suatu operator diferensial (turunan direksional) yang menunjukkan turunan $e0$ pada arah $nabla pr$. Karena arah propagasi $e0$ adalah $nabla pr$ itu sendiri, maka operasi tersebut dapat juga disederhanakan sebagai $norm(nabla pr)d/(d s) e0$ dengan $s$ jalur propagasi, sehingga didapatkan bentuk persamaan @nexthell
  ...
  atau dalam bentuk persamaan @direxp
  ...
  Selanjutnya, muka gelombang dapat diinterpretasikan sebagai lengkungan Gauss, yaitu sebuah permukaan yang terbentuk dari dua lengkungan utama. @gausscurv mengilustrasikan dua muka gelombang $cal(A)$ dan proyeksinya $cal(B)$ pada jarak $s$, masing-masing dengan kelengkungan yang ditunjukkan oleh persamaan @curv1 dan @curv2
  
  $ K_cal(A) = 1/(rho_1 rho_2) $ <curv1>
  
  $ K_cal(B) &= 1/((rho_1 + s)(rho_2 + 2) $ <curv2>

  \

  Misalkan unit vektor $hat(bup(t)) = (nabla pr) / norm(nabla pr) = (nabla pr) / n(bup(r))$ merupakan vektor unit yang menunjukkan arah propagasi pada suatu titik di muka gelombang, serta vektor $bup(u)$ dan $bup(v)$ yang menunjukkan pergerakan di sepanjang masing-masing kelengkungan muka gelombang, maka divergensi dari $hat(bup(t))$ dapat dijabarkan sebagai penjumlahan dari turunan direksional setiap basis sebarang non-ortonormal yang menyusunnya, dalam hal ini digunakan $(bup(t), bup(u), bup(v))$, sehingga didapatkan persamaan @awaw
  ...
  Persamaan @f5 ini menunjukkan adanya atenuasi ruang pada jarak $s$ dari sumber. Pada sumber gelombang bulat, maka $rho_1 = rho_2 = R$, sehingga didapakan relasi seperti yang ditunjukkan oleh persamaan @fsat2
...
Jika $delta$ mendekati 0, maka luas $S$ juga mendekati 0 sehingga sisi kanan persamaan tersebut menjadi 0, dan didapakan bentuk persamaan @f6
...
#[
  ...
  Masing-masing $bup(E)$ pada kedua sisi antarmuka berada disepanjang $cal(l)$ dan memiliki arah yang berlawanan, maka jika $hat(bup(t))$ adalah suatu vektor unit yang bersinggungan (_tangent_) dengan garis antarmuka, maka integrasi di atas menjadi persamaan @zerodot
...
Karena pada $RR^3$, vektor $bup(t)$ dapat  berupa vektor mana saja pada bidang antarmuka, maka persamaan @zerodot tersebut juga dapat ditulis sebagai relasi yang ditunjukkan persamaan @intcond
...
#[
  ...
  Medan listrik dari sebuah gelombang elektromagnetik datang ke sebuah perbatasan medium yang bergerak pada bidang $x z$ seperti pada @reftrans dapat diformulasikan sebagai persamaan @harm2

  $
    bup(E)_i = un(y)E_0 e^(-j bup(k)_i dot bup(r))
  $ <harm2>

  di mana $E_0$ amplitudo medan listrik awal sedangkan medan akibat refleksi $bup(E)_r$ dan transmisi $bup(E)_t$ pada perbatasan medium masing-masing ditunjukkan persamaan @refl1 dan @trans1
  ...
  dengan $Gamma$ dan $T$ adalah koefisien refleksi dan transmisi.

  Dengan $un(z)$ sebagai normal dari antarmuka medium, maka dari persamaan @intcond, didapatkan persamaan @continuity@stutzman_antenna_2013
  ...
  Selain itu, kontinuitas juga mengharuskan gelombang memiliki fasa yang sama pada titik refleksi, sehingga didapakan relasi yang ditunjukkan oleh persamaan @continuity1
  ...
  Karena $bup(k) = abs(bup(k))hat(bup(k))$, sedangkan $abs(bup(k))$ adalah intrinsik untuk setiap medium, sehingga @continuity1 menjadi persamaan @snells

  $ k_1 sin theta_i = k_1 sin theta_r = k_2 sin theta_t $ <snells>

  yang mana persamaan @snells tersebut terdiri atas hukum pada persamaan @snref

  $ sin theta_i = sin theta_r $ <snref>

  dan hukum refraksi seperti pada persamaan @sntra

  $ n_1 sin theta_i = n_2 sin theta_t $ <sntra>

  dengan $k_i = n_i k_0$ dan $k_0$ adalah angka gelombang pada vakum.

  \

  Selain itu, persamaan @continuity1 menunjukkan bahwa persamaan @continuity dapat disederhanakan menjadi relasi yang ditunjukkan oleh persamaan @refltrans
  ...
  Jika kembali diperhatikan persamaan gelombang listrik harmonik dengan sembarang arah osilasi dengan bentuk persamaan @simplewave

  $ bup(E) = E_0 e^(-j bup(k) dot bup(r)) $ <simplewave>

  dan kemudian jika medan pada persamaan @simplewave ini diaplikasikan ke persamaan @fourfar, akan didapatkan persamaan @fz1
  ...
  #[
    ...
    di mana $eta = sqrt(mu/epsilon)$ adalah impedansi medium.

    Mengaplikasikan persamaan tersebut ke persamaan @harm2 hingga @trans1 memberikan persamaan @magr1 hingga @magr3
    ...
    Lalu seperti halnya medan listrik, kontinuitas medan magnet juga dapat dituliskan dalam relasi yang ditunjukkan oleh persamaan @magsaysay

    $ un(z) times (bup(H)_i + bup(H)_r) &= un(z) times bup(H)_t |_(z=0) $ <magsaysay>

    \

    Selain itu, dari @reftrans, masing-masing vektor gelombang dapat diuraikan menjadi komponen-komponen unitnya yang dapat dilihat pada persamaan @xxa hingga @xxc:
    ...
    sehingga substitusi persamaan @magr1 hingga @magr3 ke persamaan @magsaysay akan memberikan persamaan @magsup
    ...
    Karena $theta_i = theta_r$ dan pada $z=0$, fasa tiap suku adalah sama, maka @magsup dapat disederhanakan menjadi persamaan @magmag

    $ 1/eta_1 (Gamma - 1) cos theta_i = - T/eta_2 cos theta_t $ <magmag>

    \

    Jika hubungan yang diberikan oleh persamaan @refltrans digunakan pada @magmag, maka didapatkan koefisien refleksi dan transmisi dengan medan listrik tegak lurus bidang refleksi sebagai persamaan Fresnel yang ditunjukkan oleh persamaan @gammaperp dan @tperp@stutzman_antenna_2013
    ...
    Dengan cara yang sama, tetapi dengan membalik polarisasi $bup(E)$ dan $bup(H)$ sehingga $bup(E)$ pada bidang refleksi dan $bup(B)$ tegak lurus, didapatkan persamaan Fresnel seperti pada persamaan @gammapar dan @tepar
...
#[
  ...
  Seperti halnya sinar refleksi dan transmisi, kekuatan medan yang direpresentasikan oleh suatu sinar difraksi juga dijelaskan oleh suatu koefisien, yaitu koefisien refraksi $D$. Penelitian Keller terhadap GTD memberikan persamaan koefisien difraksi @kellerd@keller_geometrical_1962
  ...
  dimana parameter jarak $L$ ditunjukkan oleh persamaan @utdl
  ...
  kemudian fungsi $a^plus.minus$ ditunjukkan oleh persamaan @aplmn

  $
    a^plus.minus(theta) = 2 cos((2 n pi N^plus.minus - theta)/2)
  $ <aplmn>

  dengan $N^plus.minus$ adalah bilangan bulat yang memenuhi persamaan @aplmnc

  $ 2 pi n N^+ - theta = pi "dan" 2 pi n N^- - theta = - pi $ <aplmnc>

  dan fungsi transisi $F$ merupakan fungsi transisi Fresnel berupa persamaan @tranfunc
  ...
  dengan $bup(E)_t$ medan transmisi.
  
  Pada kondisi tersebut, koefisien difraksi $D$ dapat diestimasikan sebagai persamaan @dieutd@burnside_high_1983

  $
    D_(s,h)(pa, pa') = [(1-T)D(pa - pa') + Gamma D(pa + pa')]
  $ <dieutd>

  dengan koefisien refleksi total $R$ pada persamaan @dieutdr

  $
    R = R_(perp, parallel) = (R_(1(perp,parallel))(1-P_d^2 P_a))/(1-R^2_(1,(perp,parallel))P_d^2 P_a)
  $ <dieutdr>

  koefisien transmisi total $T$ pada persamaan @dieutdt

  $
    T = T_(perp, parallel) = ((1 - R_(1(perp,parallel)))P_d^2 P_t)/(1-R^2_(1,(perp,parallel))P_d^2 P_a)
  $ <dieutdt>

  dan komponen difraksi $D$ pada persamaan @dieutdd

  $
    D(pa minus.plus pa') = (-e^(-j pi / 4))/(2 sqrt(2 pi k)) (F[k L a(pa minus.plus pa')])/cos((pa minus.plus pa')/2)
  $ <dieutdd>

  dimana $P_d$, $P_a$, dan $P_t$ didefinisikan sesuai persamaan @dieutdpd hingga @dieutdpt
...
Secara konsep, CEM dapat dibagi menjadi dua: pemodelan empiris dan pemodelan deterministik. Pemodelan empiris bergantung kepada pengamatan dan hasil pengukuran dan memberikan persamaan berupa fungsi aljabar sederhana atas frekuensi, jarak, gain, dan sebagainya untuk mengestimasi perilaku gelombang dalam kondisi-kondisi spesifik.
...
#[
  ...
  Kemudian dari persamaan @direh pada ruang hampa, dan mensubstitusikannya ke persamaan @poynting1, serta $eta_0 = sqrt(mu_0 slash epsilon_0)$, maka didapatkan persamaan @angler

  $
    angle.l bup(S) angle.r  &= 1/(2 eta_0) cal(Re)[bup(E) times dash(hat(bup(k))) times dash(bup(E))] \
  $ <angler>

  dan dengan kembali menggunakan perkalian silang tiga vektor, persamaan @angler tersebut menjadi persamaan @anglen
  ...
  Selanjutnya, karena $bup(E)$ dan $hat(bup(k))$ saling tegak lurus sehingga perkalian titiknya nol, maka @anglen menjadi persamaan @anglex

  $
    angle.l bup(S) angle.r &= 1/(2 eta_0) cal(Re)[(bup(E) dot dash(bup(E)))dash(hat(bup(k)))] \
  $ <anglex>

  sehingga intensitas atau rapat daya sebagai besar dari vector Poynting rata-rata yang terdefinisi pada persamaan @anglex menjadi persamaan @intex
  ...
  Menggunakan defini intensitas dari persamaan @intex ini, didapakan daya yang diterima sebagai persamaan @powtex@stutzman_antenna_2013

  $
    P_R = I A_"em"
  $ <powtex>

  dimana $A_"em"$ adalah apertur efektif maksimum yang terdefinisi sebagai persamaan @nextkk@balanis_balanis_2024

  $
    A_"em" = lambda^2/(4 pi) D_0
  $ <nextkk>

  dengan $D_0$ direktivitas maksimum. Maka dengan menggunakan medan listrik total dari tahap sebelumnya dan mengasumsikan penerima berupa antena isotropik dengan $D_0 = 1$, dan daya pada pemancar $P_0$, dengan menggunakan defini @nextkk, maka rugi jalur linear dapat dikalkulasi dengan persamaan @losslin
